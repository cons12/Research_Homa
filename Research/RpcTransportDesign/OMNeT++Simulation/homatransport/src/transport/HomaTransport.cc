...
uint16_t
HomaTransport::ReceiveScheduler::SchedSenders::getGrantPrioFromRawPrio(std::string mode, rawPrio){
...
HomaTransport::ReceiveScheduler::SchedSenders::getPrioForMesg(SchedState& st)
{
	...
	// rawPrio = age of flow / (message bytes sent) ^ alpha
	// return getGrantPrioFromRawPrio(homaConfig->r_mode, rawPrio)
    } else if (homaConfig->r_mode == "aware") {
	// rawPrio =  age of flow / (message bytes remaining) ^ alpha
	// return getGrantPrioFromRawPrio(homaConfig->r_mode, rawPrio)
    }
